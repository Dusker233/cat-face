    ...
    for dir_name in tqdm(os.listdir(SRC), leave=False, desc="processing"):
        ...
        for file_name in tqdm(os.listdir(src_path), leave=False, desc=dir_name):
            ...
            try:
                ...
                print(f"跳过文件 {src_file_path}，原因: {err}")
                num_skipped_photos += 1
                continue
            except Exception as e:
                print(f"跳过文件 {src_file_path}，未知错误: {e}")
                num_skipped_photos += 1
                continue
            # 过滤非cat目标
            cat_results = []
            ...
            if len(cat_results) == 0:
                ...
                continue
            # 裁剪出cat
            for i, cat_result in enumerate(cat_results):
                ...
                dest_file_path = os.path.join(
                    dest_path, f"{os.path.splitext(file_name)[0]}_{i}.jpg"
                )
